(function(root){
  ...
  function create(size=16,rng=Math.random,now=0,seconds=SECONDS[size]){if(!COUNTS.includes(size))throw Error('不支援的卡牌數量');if(seconds!==null&&(!Number.isFinite(seconds)||seconds<180||seconds>600||seconds%30!==0))throw Error('不支援的任務時間');const ids=shuffle(SYMBOLS.map((_,i)=>i),rng).slice(0,size/2);return {size,deck:shuffle(ids.flatMap(id=>[id,id]),rng),matched:[],open:[],phase:'playing',paused:false,remainingMs:seconds===null?null:seconds*1000,shields:SHIELDS[size],score:0,bonus:0,combo:0,bestCombo:0,flips:0,attempts:0,revealMs:0,lastAt:now,reason:''};}
  ...
  const api={COUNTS,SECONDS,SHIELDS,SYMBOLS,create,act};if(typeof module!=='undefined'&&module.exports)module.exports=api;else root.OrbitMemory=api;
