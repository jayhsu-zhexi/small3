...
function create(canvas,minimap){const T=root.THREE,E=root.Kart,scene=new T.Scene(),renderer=new T.WebGLRenderer({canvas,antialias:true,alpha:false,powerPreference:'high-performance'}),camera=new T.PerspectiveCamera(62,1,.1,1100),reduced=root.matchMedia('(prefers-reduced-motion: reduce)').matches;
renderer.setPixelRatio(Math.min(root.devicePixelRatio||1,1.5));renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFSoftShadowMap;renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;scene.background=new T.Color('#8a9ca8');scene.fog=new T.Fog('#a7b3bc',230,650);
scene.add(new T.HemisphereLight('#d9e5ef','#34423e',1.65));const sun=new T.DirectionalLight('#fff0dc',2.4);sun.castShadow=true;sun.shadow.mapSize.set(1024,1024);Object.assign(sun.shadow.camera,{left:-45,right:45,top:45,bottom:-45,near:1,far:160});sun.shadow.bias=-.0005;scene.add(sun,sun.target);
...
sand.map=rockTexture;sand.bumpMap=rockTexture;sand.bumpScale=.3;sand.color.set('#a4a5a0');
const environment=imageLoader.load('/assets/kart-sky.png',tex=>{if(disposed){tex.dispose();return;}tex.mapping=T.EquirectangularReflectionMapping;tex.colorSpace=T.SRGBColorSpace;scene.background=tex;scene.environment=tex;});imageTextures.push(environment);
const sea=mesh(geo(new T.PlaneGeometry(1600,1600)),material('#4aafbf',{roughness:.25,metalness:.2}),0,-3,0);sea.rotation.x=-Math.PI/2;const waterMat=new T.ShaderMaterial({uniforms:{time:{value:0},sky:{value:environment}},vertexShader:'varying vec3 v;varying vec3 world;void main(){v=position;world=(modelMatrix*vec4(position,1.0)).xyz;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}',fragmentShader:'uniform float time;uniform sampler2D sky;varying vec3 v;varying vec3 world;void main(){float w=sin(v.x*.7+time*.7)*sin(v.y*.8+time*.4);vec3 n=normalize(vec3(sin(v.x*.32+time*.5)*.055,1.0,cos(v.y*.43+time*.6)*.045));vec3 eye=normalize(world-cameraPosition);vec3 r=reflect(eye,n);vec2 uv=vec2(atan(r.z,r.x)*.15915494+.5,asin(clamp(r.y,-1.0,1.0))*.31830989+.5);vec3 reflection=texture2D(sky,uv).rgb;float fresnel=pow(1.0-max(0.0,dot(-eye,n)),3.0);vec3 c=mix(vec3(.055,.12,.15),reflection,.25+.55*fresnel);gl_FragColor=vec4(c+w*.008,1.0);}'});mats.push(waterMat);sea.material=waterMat;
...
const trees=Array.from({length:44},(_,i)=>{const a=i*2.399,r=45+(i%7)*4;return {x:Math.cos(a)*r,z:Math.sin(a)*r*.8,h:6+i%4};});
