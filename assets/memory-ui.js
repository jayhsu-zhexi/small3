(function(){
  ...
  let selected=16,duration=E.SECONDS[16],state=null,cards=[],raf=null,endTimer=null,launchTimer=null,launching=false,run=0,viewKey='',dialogMode='',particles=[],lastDraw=0,lowTimeAnnounced=false;
  ...
  for(const [level,count] of E.COUNTS.entries()){
    ...
    b.append(art,number,bars);b.onclick=()=>{if(selected!==count)duration=E.SECONDS[count];selected=count;setup()};$('memoryCounts').appendChild(b);
  }
  function setup(){for(const b of $('memoryCounts').children)b.setAttribute('aria-pressed',String(Number(b.dataset.count)===selected));const time=clock(duration===null?null:duration*1000);$('setupTime').textContent=time;$('memoryTimeSettings').setAttribute('aria-label','調整任務時間，目前 '+(duration===null?'不限時':time));$('timeRecommendation').textContent=selected+' 張卡牌・建議 '+clock(E.SECONDS[selected]*1000);if(duration===null){const icon=node('span','','cinematic-icon icon-infinity');icon.setAttribute('aria-hidden','true');$('memoryTimeValue').replaceChildren(node('span','不限時','sr-only'),icon);}else $('memoryTimeValue').textContent=time;$('timeMinus').disabled=duration===180;$('timePlus').disabled=duration===null;$('timeMinus').setAttribute('aria-label',duration===null?'改為 10 分鐘':'減少 1 分鐘');$('timePlus').setAttribute('aria-label',duration===600?'改為不限時間':'增加 1 分鐘');$('timeMinus').setAttribute('title',$('timeMinus').getAttribute('aria-label'));$('timePlus').setAttribute('title',$('timePlus').getAttribute('aria-label'));$('setupShield').textContent=E.SHIELDS[selected];$('setupPairs').textContent=String(selected/2).padStart(2,'0');}
  function adjustTime(direction){duration=direction>0?(duration===null||duration===600?null:Math.min(600,duration+60)):(duration===null?600:Math.max(180,duration-60));setup();}
  ...
  function stop(){if(raf!==null)cancelAnimationFrame(raf);raf=null;if(endTimer!==null)clearTimeout(endTimer);endTimer=null;if(launchTimer!==null)clearTimeout(launchTimer);launchTimer=null;launching=false;launchDialog.close();document.body.classList.remove('is-launching');clearEffects();}
  ...
  function start(){
    ...
    viewKey='';lowTimeAnnounced=false;document.body.classList.add('in-mission');$('memoryHome').hidden=true;$('memoryGame').hidden=false;$('memoryStatus').textContent='正在前往太空站…';$('deckLabel').textContent=selected+' 張';
  ...
  function render(){if(!state)return;const timed=state.remainingMs!==null;sound.setTension(timed?1-state.remainingMs/45000:0);$('timeValue').textContent=clock(state.remainingMs);$('timeValue').setAttribute('aria-label',timed?'剩餘時間 '+clock(state.remainingMs):'不限時間');$('timeValue').parentElement.classList.toggle('critical',timed&&state.remainingMs<=30000);$('scoreValue').textContent=state.score.toLocaleString('en-US');$('comboValue').textContent='×'+state.combo;$('shieldValue').textContent=state.shields+'/'+E.SHIELDS[state.size];$('pairsValue').textContent=state.matched.length/2+'/'+state.size/2;$('flipsValue').textContent=state.flips;const percent=state.matched.length/state.size*100;$('powerFill').style.width=percent+'%';$('powerTrack').setAttribute('aria-valuenow',String(Math.round(percent)));$('memoryPause').disabled=!['playing','resolving'].includes(state.phase);$('memoryGrid').classList.toggle('obscured',state.paused);
  ...
  function draw(now){if(!ctx)return;ctx.clearRect(0,0,window.innerWidth,window.innerHeight);particles=particles.filter(p=>now-p.at<p.life);for(const p of particles){const seconds=(now-p.at)/1000;ctx.globalAlpha=Math.max(0,1-(now-p.at)/p.life);ctx.fillStyle=p.color;ctx.fillRect(p.x+p.vx*seconds,p.y+p.vy*seconds+45*seconds*seconds,3,3);}ctx.globalAlpha=1;lastDraw=now;}
  function advance(action,now=performance.now()){if(!state)return;const result=E.act(state,action,now);state=result.state;render();for(const event of result.effects){const ended=['won','lost'].includes(event.kind);if(ended)sound.stopMusic();else sound.play(event.kind);if(event.kind==='match'){burst(event.indices,'match');$('memoryStatus').textContent='模組已連接！'+(state.combo>1?'連續 '+state.combo+' 次配對成功。':'繼續尋找下一組。');}if(event.kind==='miss'){burst(event.indices,'miss');$('memoryStatus').textContent='訊號不相符，護盾消耗 1 點。記住剛剛的位置。';}if(ended){const ticket=run;endTimer=setTimeout(()=>{endTimer=null;if(ticket===run)showDialog('result')},reduced.matches?0:700);}}schedule();}
  function frame(now){raf=null;if(state&&!state.paused&&['playing','resolving'].includes(state.phase))advance({type:'tick'},now);draw(now);schedule();}
  function schedule(){if(raf===null&&(particles.length||state&&!state.paused&&['playing','resolving'].includes(state.phase)))raf=requestAnimationFrame(frame);}
  ...
  function stat(label,value){const wrap=node('div');wrap.append(node('span',label),node('strong',String(value)));$('dialogStats').appendChild(wrap);}
  function showDialog(mode){dialogMode=mode;dialog.classList.toggle('outcome-dialog',mode==='result');dialog.dataset.outcome=mode==='result'?state.phase:'';$('resultScene').hidden=mode!=='result';$('dialogStats').replaceChildren();$('dialogResume').hidden=false;$('dialogRestart').hidden=!state;$('dialogMenu').hidden=!state;
    if(mode==='result'){const won=state.phase==='won';$('dialogTag').textContent=won?'MISSION COMPLETE':'SIGNAL LOST';$('dialogTitle').textContent=won?'太空站重新上線':'本次救援暫時中止';$('dialogCopy').textContent=won?(state.remainingMs===null?'所有模組已連接。剩餘護盾已換成額外獎分。':'所有模組已連接。剩餘時間與護盾已換成額外獎分。'):state.reason==='time'?'任務時間已耗盡。再試一次，利用記住的位置完成修復。':'護盾已耗盡。可以從較少卡牌開始，練熟後再挑戰。';stat('最終得分',state.score.toLocaleString('en-US'));stat('最高連擊','×'+state.bestCombo);stat('翻牌 / 配對嘗試',state.flips+' / '+state.attempts);stat(won?'生存獎分':'已修復模組',won?'+'+state.bonus:state.matched.length/2+'/'+state.size/2);$('dialogResume').hidden=true;$('dialogRestart').textContent='再挑戰一次';}
    else if(mode==='help'){$('dialogTag').textContent='MISSION BRIEF';$('dialogTitle').textContent='記住位置，接回能源';$('dialogCopy').textContent='每次翻開兩張卡。符號相同就鎖定完成，不同則在短暫顯示後翻回，並消耗 1 點護盾。每組配對得 100 分，連續成功每次多加 25 分（最高額外 225 分）；配錯會中斷連擊。首頁可用 −／＋ 調整時間，10 分鐘再加一次為不限時。時間或護盾歸零就結束；不限時仍會消耗護盾。完成全部配對另加剩餘每秒 2 分、每點護盾 10 分，不限時只計護盾獎分。暫停會遮住卡牌並停止倒數。';$('dialogResume').textContent=state?'返回任務':'了解，準備出發';$('dialogRestart').hidden=true;$('dialogMenu').hidden=true;}
  ...
  $('memorySound').onchange=()=>{sound.setEnabled($('memorySound').checked);$('audioStatus').textContent=$('memorySound').checked?'音效已開啟。':'音效已關閉。';};
  $('memoryMusic').onchange=()=>{sound.setMusic($('memoryMusic').checked);$('audioStatus').textContent=$('memoryMusic').checked?'背景音樂已開啟，任務進行時播放。':'背景音樂已關閉。';};
