(function(){
  ...
  function advance(action,now=performance.now()){if(!state)return;const result=E.act(state,action,now);state=result.state;render();for(const event of result.effects){sound.play(event.kind);if(event.kind==='match'){burst(event.indices,'match');$('memoryStatus').textContent='模組已連接！'+(state.combo>1?'連續 '+state.combo+' 次配對成功。':'繼續尋找下一組。');}if(event.kind==='miss'){burst(event.indices,'miss');$('memoryStatus').textContent='訊號不相符，護盾消耗 1 點。記住剛剛的位置。';}if(['won','lost'].includes(event.kind)){const ticket=run;endTimer=setTimeout(()=>{endTimer=null;if(ticket===run)showDialog('result')},reduced.matches?0:700);}}schedule();}
  ...
  function resume(){dialog.close();if(state?.paused){sound.setEnabled($('memorySound').checked);advance({type:'resume'});cards.find(b=>!b.disabled)?.focus({preventScroll:true});}}
