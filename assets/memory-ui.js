(function(){
  ...
  function audioStatus(message){$('audioStatus').textContent=message;$('audioStatus').hidden=false;}
  const sound=window.OrbitAudio.create(audioStatus,()=>{$('audioStatus').hidden=true;});
  ...
  function equipment(id){if(id>=30)return node('span',E.SYMBOLS[id][0],'equipment-art equipment-glyph equipment-symbol-'+id);const extra=id>=26,art=node('span','','equipment-art equipment-sheet-'+(extra?3:id<13?1:2)),cell=extra?id-26:id%13,columns=extra?2:4;art.style.backgroundPosition=(cell%columns*100/(columns-1))+'% '+(Math.floor(cell/columns)*100/(columns-1))+'%';return art;}
  ...
  $('memorySound').onchange=()=>{sound.setEnabled($('memorySound').checked);savePreferences();};
  $('memoryMusic').onchange=()=>{sound.setMusic($('memoryMusic').checked);savePreferences();};
