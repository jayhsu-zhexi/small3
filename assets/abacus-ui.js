(()=>{
 ...
 function nextAuto(){E.randomProblem(s,autoSettings.limit,autoSettings.operation);dirty=false;render();feedback('電腦已出題，撥好算珠後按「確認答案」。');}
 $('computerForm').addEventListener('submit',e=>{e.preventDefault();const settings={limit:Number($('autoLimit').value),operation:$('autoOperation').value};try{E.randomProblem(s,settings.limit,settings.operation);autoSettings=settings;dirty=false;$('autoError').textContent='';$('formError').textContent='';if($('problemDialog').open)$('problemDialog').close();render();feedback('電腦已出題，撥好算珠後按「確認答案」。');}catch(error){$('autoError').textContent=error.message;}});
 let draft=null;
 $('openProblem').addEventListener('click',()=>{hideEffect();draft={first:$('first').value,second:$('second').value,subtract:$('subtract').checked,dirty};$('problemDialog').showModal();});
