...
入口：https://small3.vercel.app/memory。首頁也有入口。
提供 16+／24+／32+／40+／52+，各難度依畫面補齊完整配對，最多為 18／28／36／46／60 張。基礎建議時間依序為 3:00／3:30／4:00／4:30／5:00，基礎護盾依序為 16／32／48／72／104；實際護盾及建議時間按補牌比例增加，手動選定時間不隨画面變更。時間可用圖像按鈕調整至 3–10 分鐘，10 分鐘再按＋為不限時。
每組配對 100 分，連續成功額外加分；配錯扣 1 護盾並中斷連擊。時間或護盾歸零會結束。配對得分與通關後的時間、護盾生存獎分分開顯示，延長時間不會直接增加配對得分。
包含 3D 翻牌、配對粒子光效、五種 Web Audio 合成音效、暫停／重啟，以及背景自動暫停。配對鎖定、最多 60 張小螢幕排列及所有終局皆有檢查。
重新整理會回到任務選單，並保留這台裝置上次的卡牌張數、時間、外觀和聲音開關。結算顯示命中率、翻牌次數、最高連擊，並與最近一次相同張數及時間的任務比較；本機最多記住 50 次任務，不寫入學科或樂園存檔。
立體圖像沿用原稿，以 WebP 發布，11 張合計由 23.5 MB 降至 2.7 MB；結果圖片於啟動後才載入。手機和平板會依畫面剩餘空間，將最多 60 張卡牌以完整矩形放在同一畫面；橫向、直向自動排版，旋轉不會重洗牌。完整視覺稿與規格在 `docs/memory-visual.png`、`docs/memory-design.md`。
...
自動補牌的範圍、計算與新增道具詳見 `docs/memory-plus.md`。
