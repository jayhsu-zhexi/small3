...
提供 16／24／32／40／52 張，任務時間依序為 3:00／3:30／4:00／4:30／5:00，護盾與卡牌張數相同。
每組配對 100 分，連續成功額外加分；配錯扣 1 護盾並中斷連擊。時間或護盾歸零會結束，成功則取得剩餘時間與護盾獎分。
包含 3D 翻牌、配對粒子光效、五種 Web Audio 合成音效、暫停／重啟，以及背景自動暫停。配對鎖定、52 張小螢幕排列及所有終局皆有檢查。
獨立遊戲不寫入學科或樂園存檔，重新整理會回到任務選單。完整視覺稿與規格在 `docs/memory-visual.png`、`docs/memory-design.md`。
