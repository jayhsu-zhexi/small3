...
for(let i=0;i<E.bumpers.length;i++)for(let j=i+1;j<E.bumpers.length;j++){const a=E.bumpers[i],b=E.bumpers[j];assert.ok(Math.hypot(a.x-b.x,a.y-b.y)-a.r-b.r>E.R*3,'Main bumper passages leave generous ball clearance');}
