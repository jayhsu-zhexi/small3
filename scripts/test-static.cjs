...
(async () => {
  ...
  try {
    ...
    for(const route of ['/pinball','/pinball/','/pinball.html']){const response=await fetch(origin+route);assert.equal(response.status,200);assert.match(await response.text(),/id="table"/);}
    ...
    for(const asset of ['night-market-art.js','night-market-materials-a.webp','night-market-sprites-a.webp','pinball-design.js','pinball-editor.js','pinball-editor.css','pinball-engine.js','pinball-table.js','pinball-ui.js','pinball.css','pinball-field.webp','pinball-cabinet-v2.webp','pinball-parts.webp','memory-engine.js','memory-records.js','memory-ui.js','memory-audio.js','memory.css','memory-home.css','memory-cards.css', 'memory-cinematics.css', 'memory-control-discs.webp', 'memory-result-win.webp', 'memory-result-loss.webp','memory-card-shells.webp','memory-equipment-1.webp','memory-equipment-2.webp','memory-equipment-3.webp','memory-home-v2.webp', 'memory-controls.webp', 'memory-title.webp', 'memory-home-icons.webp','memory-icons.svg','memory-background.webp','attention-ui.js','attention.css','detective-ui.js','detective.css','board-engine.js','board-ui.js','board-audio.js','board-bank.js','board-records.js','board.css','board-island.png']){
      const response=await fetch(origin+'/assets/'+asset);assert.equal(response.status,200);
      if(asset.endsWith('.webp'))assert.match(response.headers.get('content-type'),/^image\/webp/);
