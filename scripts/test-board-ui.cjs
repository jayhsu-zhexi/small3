...
function boot(saved){
  const values={...saved},elements=new Map(),timers=[],delays=[],animations=[];let document;
  function element(){
  ...
  vm.runInNewContext(fs.readFileSync('assets/board-ui.js','utf8'),{window:{PlanetBoard:E,PlanetBoardBank:bank,addEventListener(){},matchMedia(){return {matches:false}}},document,localStorage,setTimeout(fn,delay){delays.push(delay);timers.push(fn)}});
  ...
  return {get,find,direction,walk,values,timers,delays,animations,read:()=>JSON.parse(values[E.KEY])};
}
