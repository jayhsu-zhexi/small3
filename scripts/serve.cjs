...
function createServer(root = path.resolve(__dirname, '..')) {
  ...
  publicFiles.set('/detective/',['detective.html',types['.html']]);
  for(const route of ['parcels','crossing','memory','vault','survival','kart']){publicFiles.set('/'+route,[route+'.html',types['.html']]);publicFiles.set('/'+route+'/',[route+'.html',types['.html']]);}
