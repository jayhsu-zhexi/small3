...
訊息改為台面下方獨立固定高度列，手機版配置扣除該列高度，橫向時放在左側資訊區。紫色小球區補上頂部與側邊牆，只保留底部開口供進出；高架坡道仍為獨立層。右側增加隨台面輪廓變化的邊界，僅發射中的球能使用黃色發射道，一般球不能穿入黑色外緣。


## 元件架構重製（取代設計稿底圖）
assets/pinball-table.js 是球台幾何的單一來源：每個元件有 id/type/layer 與 points、shape 或 pivot/length/radius。walls/side rails/deflectors 由元件編譯成 railColliders；畫面逐元件繪製同一批 points/radius。圓形碰撞與元件共用 shape，彈力取自 kick；發射道與高架坡道共用路徑；擋板狀態由元件的軸心/長度/厚度建立；三個落球感應區也有獨立定義。

assets/pinball-ui.js 已移除整張機械底圖與固定坡道圖像遮罩。僅使用原創 pinball-cabinet-v2.webp 作為不含機械障礙的裝飾底色，軌道、牆體、紫色小球區牆面、感應通道、救援艙、圓形撞擊元件、三角板、雙擋板均獨立繪製。圓形與球體材質使用原有元件圖集並裁切到真實圓形尺寸。高架層使用共享路徑畫出，再在其上畫高架球；地面球在下層。物理保留被動牆面與中央彈力球規則。

回歸驗證除了既有球路，新增元件id唯一、碰撞端點與渲染物件的參照一致、路徑/圓形/落球區共享，以及不再載入整張機械底圖的檢查。前述『固定元件版本』僅為歷史紀錄，已被本節架構取代。
